...
| AUTH_SECRET           | 登录 cookie 签名密钥      | 随机字符串（`openssl rand -base64 32`） | 空，未设置时回退用 PASSWORD 签名；更换生效中的密钥会使所有登录失效                                                                                                      |
| LOGIN_RATE_LIMIT      | 是否启用登录失败限速      | true/false                              | false（默认关闭）。仅当反向代理会转发真实客户端 IP（X-Forwarded-For / X-Real-IP）时才建议开启；直连 Docker/局域网请勿开启，否则所有客户端会被视为同一网关 IP 而互相牵连 |
